...
const redIcon = L.divIcon({
  ...
  html: `<span style="${markerHtmlStyles2}" />`
          ...
          // Function to check if a point is inside a polygon
            ...
            .then(data => { for (let r of data) {
              ...
              var student;
                ...
                student.bindPopup(stud +" is inside LGU San Mateo!").openPopup();
                ...
                if (distance <= 0.005) {
                student = L.marker([lat, long]).addTo(map);
                student.bindPopup(stud +" is within 500 meters from LGU San Mateo!").openPopup();
            ...
            .then(data2 => { for (let c of data2) {
              ...
              var stat = c.status;
                    ...
                    student.bindPopup(stud + " is 500 meters away from LGU San Mateo!").openPopup();
    ...
    var polygon = L.polygon([
      ...
      polygon.bindPopup("LGU San Mateo.");
    ...
    function onMapClick(e) {
        popup
        .setLatLng(e.latlng)
        .setContent("It is outside LGU San Mateo! " + e.latlng.toString())
        .openOn(map);
    }
...
if (is_in_polygon(4, $vertices_x, $vertices_y, $student_longitude, $student_latitude)) {
    echo "<script>console.log('".$name." is inside LGU San Mateo!');</script>";
} else {
    ...
    if ($distance <= 500) {
        // Send alert
        echo "<script>console.log('".$name." is within 500m of LGU San Mateo!');</script>";
        ...
        if($select_student->rowCount() > 0){
            echo "<script>console.log('".$name." is already recorded on the list of student outside LGU San Mateo!');</script>";
        }else{
        //record student data
       ...
       $insert_student->execute([$id, $name, '0']);
        ...
        if($row['status']==0){
            ...
            echo "<script>console.log('".$name."  is 500m away from LGU San Mateo!');</script>";
            $url = 'https://semaphore.co/api/v4/messages';
            $data = array(  'apikey' => '7026c9e6d4b3eddee2202da4f6f9b141', //Your API KEY
                    'number' => $number,
                    'message' => $name.' is 500m away from LGU San Mateo!',
            ...
            echo "<script>console.log('".$name."  is allowed to go 500m away from LGU San Mateo!');</script>";
        }
